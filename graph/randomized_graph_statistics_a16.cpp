...
// 테스트 케이스에 대한 동일한 결과 출력
static unsigned long int randNext = 1;

int rand(void) { // RAND_MAX는 32767로 가정
...
// 그래프의 세 가지 타입
enum RESULT {
    VALID,          // 음수 가중치 사이클이 있는 경우
    INVALID,        // 에지가 하나도 없는 정점이 존재하는 경우
    INTERESTING     // 그래프의 평균 거리가 가장 큰 에지 가중치의 절반보다 작은 경우
};
...
int main() {
    ...
    double percenInteresting = (double)interesting / valid * 100;

    cout << "유효하지 않은 그래프 개수: " << invalid << endl;
    cout << "흥미로운 그래프 생성 비율: " << fixed << setprecision(2) << percenInteresting << "%" << endl;  // 소수점 2자리까지 출력
